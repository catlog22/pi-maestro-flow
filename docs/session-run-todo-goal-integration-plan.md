...
> Goal 契约更新（2026-07-16）：本文中的 `goal set/pause/done` 工具动作已被取代。LLM 工具仅提供 `get/create`；用户通过 `/goal stop`、`/goal resume`、`/goal clear` 控制生命周期；正常 `agent_end` 自动验证，`turn_end` 与 `session_shutdown` 不触发验证。
...
关键转变与 guide 的"注册从 push 变 pull"同构：**镜像从"LLM 手工声明"变"插件读盘派生"**。LLM 在 canonical Workflow 中只保留以下宿主级动作：

- `todo({action:"next"})` —— 步进（激活下一步 + 注入上下文）
- `goal({action:"get"})` —— 只读 Goal 投影；完成时让 agent loop 自然结束，由 `agent_end` 自动触发前置校验与 verifier

`goal({action:"create"})` 仅用于没有 canonical Workflow Goal 的独立长时目标。LLM 不拥有 stop/resume/clear/done；用户通过 `/goal stop`、`/goal resume`、`/goal clear` 控制生命周期。
...
| goal 字段 | 派生来源 | 说明 |
|---|---|---|
| objective | `session.json.intent` + `boundary_contract.definition_of_done` | bridge 检测到 active session 且无 Goal 时自动物化（或提示用户确认） |
| tokenBudget | `orchestration.quality_mode` 经项目配置映射，不在插件内硬编码档位数值 | 预算暂停即现成的成本闸门 |
| 自动验证前置校验 | `orchestration.chain` 全 completed 且 session 级 gates 无 failed | 正常 `agent_end` 后先检查 canonical blockers；链未走完时跳过完成判定并继续执行，不浪费 verifier 调用 |
...
1. `onAgentEnd` 在正常 agent loop 结束后执行 bridge 前置校验与 verifier；`turn_end` 不验证，`session_shutdown` 只持久化。
...
4. 新增 pauseReason `"gate"`：bridge 检测到 run 进入 blocked（exit gate failed）时**只产出 diff 事件**，由 Coordinator 裁决并执行内部 `pauseGoal(reason:"gate")` + notify——把"门禁失败"从 LLM 自觉上报变成确定性流程中断（bridge 只报告不决策，见 §零裁决 3）。
5. LLM schema 收敛为单根 `type:"object"` 的 `get/create`；`create.objective` 的条件必填由执行层校验，避免 provider 拒绝根级 `anyOf`。
...
- 把 ralph/maestro/odyssey 中各自为政的 `<goal_tracking>` 块收敛为统一 `<host_mirror>` 协议段（单源文件，各 skill 引用不复述——同 run-mode.md 的协议单源原则），内容只剩：**步进用 `todo({action:"next"})`；完成时自然结束 agent loop；状态对账与 verifier 由插件自动完成，禁止手工 update 镜像**。
...
| 门禁失败 | bridge 产出 blocked diff 事件 → Coordinator 执行内部 `pauseGoal(reason:"gate")` + statusline ⚠ + notify | 自动中断 |
...
| 用户中断 | `/goal stop` + staleToolCallsBlocked；扩展：mirror 任务 in_progress 但 run 已 sealed → tool_call 警告 | 守卫 |
...
每期配 focused tests：bridge 对账（新建/失败/重试 run 的镜像收敛）、todo v4 迁移（按既有 contract-test spec）、`agent_end` 自动验证矩阵、Goal schema provider 兼容性、`fmtStatusLine` 快照。
...
9. **Stop/Cancel contract**：保持 canonical schema 真实；用户 `/goal stop` 只暂停宿主投影、fence continuation 并 abort 当前 agent loop，CLI 不支持的 Run cancel 由 adapter 明确拒绝，不在 Goal/TUI 中伪造 `cancelled`。
...
### Goal 输入栏联动补充（2026-07-16）

- 独立 `goal-panel` 使用 `setWidget(..., { placement: "aboveEditor" })`，不依赖 canonical Workflow 是否存在。
- 生命周期状态 `ACTIVE / WAITING / VERIFYING / VERIFIED / STOPPED / BUDGET / BLOCKED / ERROR` 与 Goal 状态更新使用同一刷新 seam。
- 宽屏最多两行，显示目标、轮次、耗时、Token 与预算进度；窄屏退化为单行，宽度 1..120 不溢出。
- `/goal clear`、`session_shutdown` 与 verified 展示超时都会同时清理 statusline 和 widget，避免悬挂旧状态。
- Goal entry 记录 Pi `sessionId`；`session_start(reason:new|fork)` 强制空 Goal，同 ID 的 `resume|reload|startup` 才允许恢复。
- Goal active state 与 agent loop ownership 分离：恢复或 inconclusive 后显示 `WAITING`，普通用户输入不触发 verifier；create、`/goal resume`、内部 continuation 才能 arm 下一 loop。
- `/new`、`/fork` 不自动 attach running canonical Workflow，也不重建其 Goal 投影；用户在 `/maestro-session` 中显式 Resume 后才获取 lease 并重新启用 projection。
- `reason:startup` 不能单独作为恢复依据；必须同时命中当前 sessionId 的 Goal entry。仅按 cwd 发现的 running Workflow 先作为只读 baseline，避免每次重启 Pi 都重建 Goal。
