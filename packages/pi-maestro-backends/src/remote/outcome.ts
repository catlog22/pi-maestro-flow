 ...
 * would be free to deliver callbacks into the replacement attempt. Two of the
 ...
 * handling "we dropped the connection".
 *
...
export interface RemoteOutcomeInput {
  ...
  /** The channel dropped before `run/result` arrived. Only the caller knows this; the event stream alone cannot show it. */
 ...
 *
 * The two `unreaped` reasons stay separate on purpose. "The remote lost the
 * run" and "we lost the connection" are different faults with different
 * handling — the first says a live runtime may still be acting on the remote
 * host, the second says we simply stopped watching one — and a single merged
 * sentence would leave an operator unable to tell which happened.
 *
 * @param resultEvent - the run's final `run/result`, when one arrived.
 * @param disconnectedBeforeResult - the channel dropped before the result arrived.
