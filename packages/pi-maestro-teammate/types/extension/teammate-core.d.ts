...
export declare function deferAgentContextMessage(agent: ActiveAgent, message: string): void;
/** Atomically reserve deferred context for one substantive delivery. */
export declare function takeDeferredAgentContext(agent: ActiveAgent): string[];
/** Restore a failed delivery ahead of status messages that arrived meanwhile. */
export declare function restoreDeferredAgentContext(agent: ActiveAgent, messages: readonly string[]): void;
/** Attach one reserved status-context snapshot to a substantive delivery. */
export declare function messageWithDeferredAgentContext(context: readonly string[], message: string): string;
/** Empty successful follow-up turns update lifecycle state but need no model notification. */
export declare function shouldPublishAdditionalTurn(result: SingleResult): boolean;
