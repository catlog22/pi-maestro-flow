...
Modes: "steer" (default) | "follow_up" | "abort". Steer requests cancellation of the active agent turn and, after cancellation is acknowledged, delivers the message as the replacement or next prompt; it is never inserted into the middle of a running tool call. Omit mode only when interruption is intended. Follow_up does not interrupt and must be selected explicitly when the target should finish its current turn first; it is consumed only when the target AgentSession would otherwise stop: the active model response plus every tool call, continuation, native retry, and compaction in that turn must finish first. A tool returning is not a delivery boundary; earlier queued input is consumed first, and a session that never reaches its stop point can delay follow_up indefinitely. An unacknowledged steer degrades to queued follow_up. Cross-session targets support only "steer" and "follow_up".
...
export const TEAMMATE_SEND_GUIDELINES = [
  "Use teammate-send only for new information, a correction, an explicitly requested response, a safety/lifecycle constraint, or termination. Omitted mode defaults to steer; pass follow_up explicitly when current work must not be interrupted, and use abort only to terminate work.",
