...
export function renderTeammateCall(
  ...
  const tasks = args.tasks as unknown[] | undefined;
  let label: string;
  if (args.agent) {
    label = theme.fg("accent", args.agent as string);
  } else if (tasks?.length) {
    label = theme.fg("accent", `${tasks.length} tasks`);
  } else {
    label = theme.fg("accent", "?");
  }
  const asyncLabel =
    args.background === false ? "" : theme.fg("dim", " [bg]");

  return new Text(
    `${theme.fg("toolTitle", theme.bold("teammate "))}${label}${asyncLabel}`,
    0,
    0,
  );
}
...
export function renderTeammateResult(
  ...
  for (const singleResult of details.results) {
    ...
    const modelLabel = singleResult.model && singleResult.model !== "unknown" ? ` ${theme.fg("dim", singleResult.model)}` : "";
    const header = `${icon} ${theme.bold(singleResult.agent)}${modelLabel}`;
    lines.push(header);
