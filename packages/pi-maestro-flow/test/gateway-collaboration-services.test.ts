...
test("shared catalog exposes eight tools and Session/Todo/Monitor collaborate without Pi Todo coupling", async (t) => {
  ...
  assert.deepEqual(runtime.catalog.list().map((tool) => tool.name), ["host", "exec", "job", "file", "teammate", "session", "todo", "monitor"]);
  ...
  assert.equal(started.ok, true); const handle = (started.data as { taskId: string }).taskId;
  ...
  assert.equal((terminal.data as { task: { status: string } }).task.status, "cancelled");
