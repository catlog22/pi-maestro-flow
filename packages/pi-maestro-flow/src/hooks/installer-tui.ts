...
export class MaestroHookInstallerOverlay implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const preset = this.custom ? `${this.basePreset} · 自定义` : this.basePreset;
    const rows = [
      fitLine(`${this.params.theme.bold("Maestro Flow Hooks 安装")} · ${preset} · ${this.selectedNames.size}/${this.params.snapshot.definitions.length}`, inner),
      this.params.theme.fg("dim", fitLine("预设：1 None · 2 Minimal · 3 Standard · 4 Full", inner)),
      rule(inner),
    ...
    if (selected) {
      ...
      rows.push(this.params.theme.fg("dim", fitLine(
        `${selected.event}${selected.matcher ? ` [${selected.matcher}]` : ""} · ${selected.level}${selected.requiresWorkspace ? " · 需要 Maestro workspace" : ""}`,
        inner,
      )));
      rows.push(fitLine(`maestro hooks run ${selected.name}`, inner));
      if (selected.permissionAdvisory) {
        rows.push(this.params.theme.fg("warning", fitLine("△ Pi 中仅 advisory · 工具权限仍由 permission controller 决定", inner)));
      }
    }
    if (this.params.snapshot.thirdPartyHandlers > 0) {
      rows.push(this.params.theme.fg("dim", fitLine(`保留 ${this.params.snapshot.thirdPartyHandlers} 个非 Maestro Hook`, inner)));
    }
    if (this.params.notice) rows.push(this.styledNotice(this.params.notice, inner));
    if (this.discardArmed) {
      rows.push(this.params.theme.fg("warning", fitLine("△ 有未应用修改 · 再按一次 Esc 放弃", inner)));
    }
    rows.push(fitLine("Esc 关闭 · ↑↓ 选择 · / 筛选 · Space 切换 · A 应用 · U 卸载", inner));
  ...
  private renderCompact(width: number): string {
    ...
    const text = selected
      ? `Esc · Hooks 安装 · ${this.selectedNames.has(selected.name) ? "选" : "未选"} · ${selected.name}`
      : "Esc · Hooks 安装 · 没有匹配项";
  ...
  private definitionRows(
    ...
    if (definitions.length === 0) return [this.params.theme.fg("warning", fitLine("○ 没有匹配的 Maestro Hook", width))];
    ...
    return definitions.slice(start, start + MAX_VISIBLE).map((definition, offset) => {
      ...
      const state = checked ? this.params.theme.fg("success", "● 选择") : this.params.theme.fg("dim", "○ 跳过");
      const name = isSelected ? this.params.theme.bold(definition.name) : definition.name;
      return fitLine(
        `${cursor} ${state} · ${name} · ${definition.event} · ${definition.level}${installed.has(definition.name) ? " · 已安装" : ""}`,
        width,
      );
    });
  }
  ...
  private filterLine(width: number, count: number): string {
    const text = this.filterActive
      ? `筛选中：${this.query || "输入名称、事件或级别"} · Esc 取消`
      : "筛选：按 / 输入关键词";
  ...
  private styledNotice(notice: string, width: number): string {
    ...
    const role = /(失败|错误|failed|error)/i.test(safeNotice) ? "error"
      : /^(已安装|已卸载|已保存)/.test(safeNotice) ? "success"
      : "warning";
