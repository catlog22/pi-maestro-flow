  ...
  <script type="module">
    ...
    bridge.oncalltool = async (params) => {
      if (!consentGranted) {
        ...
        if (!accepted) {
          await post("/proxy/ui/consent", { approved: false }).catch((error) => {
            console.debug("Failed to report denied consent", error);
          });
          return {
            isError: true,
      ...
      await post("/proxy/ui/message", {
        ...
        console.debug("Failed to report tool-call intent", error);
      });
      return result;
    };
    ...
    window.addEventListener("message", async (event) => {
      ...
      if (msgType === "notify" || msgType === "prompt" || msgType === "intent" || msgType === "message") {
        ...
        await post("/proxy/ui/message", { type: msgType, ...directFields, ...(payload || {}) }).catch((error) => {
          console.debug("Failed to forward UI message", error);
        });
        ...
        await post("/proxy/ui/message", {
          ...
          console.debug("Failed to forward custom UI intent", error);
        });
      }
    });
    ...
    eventSource.addEventListener("host-context", (event) => {
      try {
        ...
        console.debug("Failed to apply host context", error);
      }
    });
    eventSource.addEventListener("session-complete", async () => {
      await bridge.teardownResource({}).catch((error) => {
        console.debug("Failed to tear down completed resource", error);
      });
      eventSource.close();
      window.close();
    });
    ...
    const heartbeat = setInterval(() => {
      post("/proxy/ui/heartbeat", {}).catch((error) => {
        console.debug("Failed to send UI heartbeat", error);
      });
    }, 10000);
    ...
    const complete = async (reason) => {
      try {
        ...
        console.debug("Failed to report UI completion", error);
      }
      try {
        await bridge.teardownResource({});
      } catch (error) {
        console.debug("Failed to tear down UI resource", error);
      }
