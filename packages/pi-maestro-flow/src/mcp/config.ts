...
export function writeManagedServerEntry(
  filePath: string,
  ...
  const raw = readRawConfigObject(filePath);
  const servers = getServersObject(raw);
  if (previousName && previousName !== serverName) delete servers[previousName];
  servers[serverName] = entry;
  setServersObject(raw, servers);
  writeRawConfigObject(filePath, raw);
  return filePath;
...
export function deleteManagedServerEntry(filePath: string, serverName: string): string {
  const raw = readRawConfigObject(filePath);
  const servers = getServersObject(raw);
    ...
    if (loaded.imports?.length) {
      for (const importKind of loaded.imports) {
        ...
        try {
          ...
          logger.debug("Skipping unreadable imported MCP config", {
            path: importPath,
