...
export function buildEnrichmentPrompt(inputs: readonly EnrichmentInput[]): string {
  const header = [
    ...
    "",
    "STEP 1 — Knowledge-moment gate (judge before the four quality classes):",
    "  A knowledge moment is a turn whose EVENT STRUCTURE carries reusable signal, not just process narration. Recognize these event structures:",
    "  - failure-then-recovery: a failed tool call (non-ok episode: failure_recovery / repeated_failure / permission_block / empty_then_refined) followed by a fix or workaround",
    "    When a trajectory episode shows a failed dev tool (bash/read/edit/grep/...), extract the CAUSAL CHAIN into the `knowledge` field:",
    "    WHICH tool failed → WHY (the error message: 'No matches', 'EACCES', 'timeout', ...) → WHAT was learned (the workaround/fix/prevention).",
    "    A bare 'grep failed' without the why and the lesson is process noise, not knowledge.",
    "  - decision-landed: the assistant states a decision and the reason (\"decided X because Y\", \"决定采用 X 因为 Y\")",
    "  - pitfall-discovered: the assistant names a non-obvious failure mode and its prevention (\"gotcha: ... must ...\", \"陷阱：... 必须 ...\")",    "  If none of these event structures is present, set worthCapturing=false — the turn is process narration, not a knowledge moment.",
    "",
    "NEGATIVE LIST (never a knowledge moment — always worthCapturing=false):",
    "  - process narration: \"I checked ...\", \"全部完成\", \"让我检查 ...\", \"工作树是干净的\", \"全部通过\"",
    "  - system logs: model-failover notices, teammate completion/delivery notices, raw error headers (these are dropped at collection; if one reaches you, still reject)",
    "  - progress reports: pure status updates with no decision/root-cause/pitfall content",
    "",
    "STEP 2 — If the knowledge-moment gate passes, judge whether the signal is one of:",
