 ...
 * - agent://<correlationId-or-name>[/json/path] — 已完成 teammate 子代理的结构化输出
 *   （数据源：teammate/agent-output-store.ts，tool_result 钩子捕获持久化）
...
import { getAgentOutputPath, readAgentOutput } from "../teammate/agent-output-store.ts";

export const ResourceParams = Type.Object({
  uri: Type.String({
    description:
      "协议资源 URI：pr://owner/repo/N（或 pr://N 当前仓库，可加 /diff /files）、issue://owner/repo/N、skill://name、rule://name、agent://<id>[/json/path]（已完成的 teammate 结构化输出，按 correlationId 或任务 name，可选 JSON 路径如 /findings/0/path）。",
  }),
});
...
export async function resolveResource(uri: string, cwd: string, signal?: AbortSignal): Promise<{ content: string; title: string; cached: boolean }> {
  ...
  switch (scheme) {
    ...
    case "agent": {
      ...
      if (!id) throw new Error('Invalid agent:// URI. Expected agent://<correlationId-or-name>[/json/path].');
      const record = await readAgentOutput(id, cwd);
      const value = pathSegments.length === 0
        ? record.output
        : (() => {
            const hit = getAgentOutputPath(record.output, pathSegments);
            if (!hit.hit) throw new Error(`agent://${id} path miss: ${hit.reason}`);
            return hit.value;
          })();
...
- \`agent://<correlationId-or-name>[/json/path]\` — structured output of a completed teammate subagent (captured when a task finishes with an outputSchema). Optional JSON path pulls one field, e.g. \`agent://reviewer-1/findings/0/path\`.
...
Read local files with the built-in read tool — resource is for protocol resources only.`,
    ...
    promptGuidelines: [
      ...
      "After a teammate task completes with an outputSchema, read individual fields via agent://<id>/<json-path> instead of re-running the agent.",
    ],
