...
const DEFINITIONS: readonly SettingDefinition[] = [
  ...
  ...["api.manage", "api.configure", "api.retry", "api.list"].map((key, index): SettingDefinition => ({
    key,
...
export function createApiManagerSettingsProvider(
  ...
  const load = async (): Promise<{
    ...
    return { providers, modelsRoot, retry, modelsContent: await readContent(modelsPath), settingsContent: await readContent(settingsPath) };
  };
  ...
  return {
    ...
    prepare: async (request) => {
      ...
      if (changedKeys.includes("api.providers") || changedKeys.includes("api.retry.enabled")
        || changedKeys.includes("api.retry.maxRetries")) {
