...
Generate LLM actions for a Skill, call Agy/Qwen/Codex through teammate unified interface for analysis or generation.
...
async function callLLM(tool, prompt, mode, timeout) {
  ...
  const command = \`teammate({ agent: "delegate", taskType: "analysis", task: "\\${escapePrompt(prompt)}", /* --to \${tool}: set model via model-availability */ }) \${modeFlag}\`;
