...
**Medium/High complexity**: Multi-prompt `teammate({ agent: "explorer" })` (preferred for read-only scan):
...
**Fallback** (when deeper analysis needed per perspective): `teammate({ agent: "delegate", taskType: "analysis", task: "<prompt>" })
