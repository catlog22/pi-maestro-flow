...
```bash
teammate({ agent: "delegate", taskType: "development", task: "PURPOSE: Apply tech debt fixes in batch; success = all items fixed without breaking changes\nTASK: <batch-type-specific-…", cwd: ""<worktree-path>" })
```
