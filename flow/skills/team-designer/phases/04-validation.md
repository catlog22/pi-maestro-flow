...
function validateCoordinatorRunLifecycle(teamConfig) {
  ...
  if (!coordinatorMd.includes('maestro run start --platform pi')) {
    results.errors.push('coordinator/role.md missing Run Lifecycle Integration (maestro run start --platform pi)');
  }
