...
if (paperPath) {
  ...
  for (const strategy of strategyMatrix) {
    if (strategy.responseType === 'Defend' || strategy.responseType === 'Clarify') {
      ...
      const cliCommand = `teammate({ agent: "delegate", taskType: "analysis", task: "PURPOSE: Search paper content for evidence supporting response to reviewer comment\n\nREVIEWER COMMENT:\n${strategy.issu…", prompt: "analysis-trace-code-execution", /* --to agy: set model via model-availability */ })
