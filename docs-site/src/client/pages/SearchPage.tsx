...
import { searchGuides, guideCategories, type SearchResult } from '@/client/routes/route-config.js';
...
export default function SearchPage() {
  ...
  const handleSearch = (searchQuery: string) => {
    ...
    if (searchQuery.trim().length >= 1) {
      setResults(searchGuides(searchQuery, categoryFilter));
    } else {
      setResults([]);
    }
  };
  ...
  const handleCategoryChange = (cat: string | undefined) => {
    ...
    if (query.trim().length >= 1) {
      setResults(searchGuides(query, cat));
    }
  };
    ...
    <div>
      <div className="mb-[var(--spacing-8)]">
        ...
        <div className="flex flex-wrap gap-[var(--spacing-2)] mt-[var(--spacing-3)]">
          ...
          {guideCategories.map((cat) => (
            <FilterChip
              key={cat.id}
              label={isZh ? cat.title_zh : cat.title}
      ...
      {results.length > 0 && (
        <div className="space-y-[var(--spacing-2)]">
          {results.map((result) => (
            <SearchResultItem key={result.slug} result={result} isZh={isZh} />
          ))}
        </div>
      )}
  ...
  return (
    ...
    >
      <div className="flex items-start gap-[var(--spacing-3)]">
        <span className="shrink-0 px-[var(--spacing-2)] py-[1px] text-[length:10px] font-[var(--font-weight-semibold)] rounded-full text-accent-blue bg-tint-blue">
          指南
        </span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-[var(--spacing-2)] mb-[var(--spacing-1)]">
            ...
            <span className="text-[length:var(--font-size-xs)] text-text-tertiary">
              {isZh ? result.categoryZh : result.category}
            </span>
          </div>
          <p className="text-[length:12px] text-text-secondary line-clamp-2">
            {isZh && result.descriptionZh ? result.descriptionZh : result.description}
          </p>
        </div>
      </div>
    </a>
