...
import { TuiSimulator } from './TuiSimulator.js';
...
const components: Components = {
  ...
  code({ className, children, ...props }) {
    ...
    if (lang === 'tui-sim') {
      const simPreset = String(children ?? '').trim() || 'settings';
      return <TuiSimulator preset={simPreset} />;
    }
    if (lang === 'mermaid') {
  ...
  pre({ children, ...props }) {
    ...
    const isTuiSim = child != null && 'tagName' in child && child.tagName === 'code' && classList?.[0]?.includes('tui-sim');
    if (isMermaid || isTuiSim) return <>{children}</>;
