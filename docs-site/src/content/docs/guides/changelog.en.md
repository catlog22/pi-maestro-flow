...
> **Current stable release: v0.22.0 (2026-08-20).** Teammate 2.0.0 backend registry (breaking major, first public 2.x), MCPX dashboard, prompt enhance, `/notify` toasts, usage insights; engine synced to `maestro-flow@0.5.79`; bundles Cockpit 0.17.0 and Settings-Core 0.2.0. Follow-up 0.22.1 / 0.22.2 are packaging-fix patches (published to npm, see below) that do not change the install command or announcement banner.

## Unreleased (next version, in development)

> The items below are a snapshot of changes merged to `master` after `v0.22.0` but not yet released as a tagged version. Entries accumulate as commits land and are promoted to a formal section on release.
...
- **Browser bridge (optional real-browser takeover)**: add an optional Chrome MV3 extension bridge that takes over the user's daily browser over WebSocket (preserving login state, CAPTCHA, real fingerprint, and extension-only APIs), auto-falling back to the existing puppeteer CDP attach/launch path when the extension is absent, with no behavior change. The extension ships in `optional/browser-bridge/`, a `/install browser-bridge` item (with probe) is added, and `BROWSER-BRIDGE-SETUP.md` documents it. The `PI_BROWSER_BRIDGE_PORT` env var customizes the WS port (default `19222`).
...
- **Released patches 0.22.1 / 0.22.2 (2026-08-21, packaging fixes only, npm `latest`)**: 0.22.1 declares flow's runtime dependency on `pi-maestro-backends` (previously installed only transitively via teammate); 0.22.2 switches the `maestro-flow` engine constraint from an exact pin `0.5.79` to a caret `^0.5.79` (the engine is pure-patch cadence, backward compatible; bumping an engine patch no longer requires changing the flow pin; cockpit/teammate/settings-core stay exact-pinned, since under 0.x a caret is a false relaxation for minor-jumping packages). Pure packaging fixes, so the install command and banner stay at `@0.22.0`.
