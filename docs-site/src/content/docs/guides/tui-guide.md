...
`/notify` 是一个 toast 提醒开关，控制模型出错与回合完成时是否弹出 toast 通知。

| 命令 | 说明 |
|------|------|
| `/notify` | 裸命令，切换 master 开关 |
| `/notify on` / `off` | 直接开 / 关 |
| `/notify error` | 仅在模型出错时提醒 |
| `/notify complete` | 仅在回合完成时提醒 |
| `/notify status` | 查看当前状态 |

- 出错的回合会抑制完成提醒，保证每回合至多一条 toast。
- 状态持久化，重新加载后保持。

## 12. 下一步建议（next-suggest）

开启后，每个 agent 回合结束会在编辑器下方渲染一个「下一步提示」widget：

- `F2`（可配）填充编辑器以接受建议；任意输入即关闭 widget。
- 配置入口为 API Manager 的 `nextsuggest` 子命令（`/api-manager nextsuggest`），持久化于 `api-manager.json` 的 `nextSuggest` 段，可设开关、模型 pin、思考深度、长度上限与接受键。
