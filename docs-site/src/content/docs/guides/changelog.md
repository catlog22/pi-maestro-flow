...
- **浏览器扩展桥（显式有限通道）**：新增可选 Chrome MV3 extension channel，以首帧 token 认证连接用户 Chrome；必须通过 `app.channel:"extension"` 显式选择，断连和 unsupported API 都 fail closed，不自动接管或回退。首版 adapter 支持 URL/title、goto/evaluate、raw CDP/batch、cookies、tabs 与 CDP screenshot，不承诺完整 Puppeteer parity。`browser status` 提供 live server/认证连接/tab 与命名 entry 元数据；`/install` probe 只检查 historical verified marker + 合法配置。扩展与安装指南随包发布在 `optional/`。
