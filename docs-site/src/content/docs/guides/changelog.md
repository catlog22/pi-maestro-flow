...
> **当前状态：v0.17.0 候选版本。** 本页以 `v0.16.0` 标签（`02436592`）为基线，整理截至 `8e4c3d38` 的 37 个后续提交，并纳入当前工作区中的 v0.17.0 版本清单。正式标签发布前，npm 安装命令和最终校验数据仍以发布结果为准。
...
这项变化主要改善长时间、多窗口和跨会话工作流。相关概念见 [并行多智能体调度](/guides/teammate-dispatch)、[Advisor 逐轮监督](/guides/advisor)和 [Pi Cockpit 可视化](/guides/cockpit)。
