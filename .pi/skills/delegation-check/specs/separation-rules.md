...
Rules for validating the boundary between **command delegation prompts** (teammate() calls) and **agent role definitions** (agent `.md` files). Derived from analysis of GSD's `plan-phase.md`, `execute-phase.md`, `research-phase.md` and their corresponding agents (`gsd-planner`, `gsd-plan-checker`, `gsd-executor`, `gsd-phase-researcher`, `gsd-verifier`).
...
teammate({ agent: "gsd-plan-checker", prompt: "You are a code quality expert. Your job is to review plans...
    <objective>Verify phase 5 plans</objective>" })

# GOOD — prompt states objective only
teammate({ agent: "gsd-plan-checker", prompt: "<verification_context>
...
teammate({ agent: "gsd-planner", prompt: "<objective>Create plans for phase 3</objective>
    Remember: tasks should have 2-3 items max.
    | TOO VAGUE | JUST RIGHT |
    | " })

# GOOD — agent's own <task_breakdown> section owns this knowledge
teammate({ agent: "gsd-planner", prompt: "<planning_context>
    <files_to_read>...</files_to_read>
  </planning_context>" })
```
...
teammate({ agent: "gsd-planner", prompt: "Step 1: Read the roadmap. Step 2: Extract requirements.
    Step 3: Create task breakdown. Step 4: Assign waves..." })

# GOOD — prompt states objective, agent decides process
teammate({ agent: "gsd-planner", prompt: "<objective>Create plans for phase 5</objective>
    <files_to_read>...</files_to_read>" })
```
...
teammate({ agent: "gsd-planner", prompt: "Use React Query for data fetching. Use Zustand for state management.
