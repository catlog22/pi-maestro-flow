...
|---------|-----------|--------|
| CLI | Complex multi-file changes | `teammate({ taskType: "development", /* --to agy */ })` |
| Direct | Simple single-file changes | Inline Edit/Write |

### CLI Implementation (Complex)

```
Bash(`teammate({ agent: "delegate", taskType: "development", task: "PURPOSE: Implement loading state and error handling for upload form\nTASK:\n  - Add useState for isLoading and error\n  …" })
```
