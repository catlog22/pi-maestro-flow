...
todo({ action: "update" })({ taskId: "REFACTOR-B{NN}", addBlockedBy: ["DESIGN-001"], owner: "refactorer" })
...
todo({ action: "update" })({ taskId: "VALIDATE-B{NN}", addBlockedBy: ["REFACTOR-B{NN}"], owner: "validator" })
...
|-------|--------|----------|
| Task count correct | todo({ action: "list" }) count | single: 5, auto/fan-out: 2 (pre-CP-2.5), independent: 5*M |
