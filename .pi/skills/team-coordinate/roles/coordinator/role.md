...
2. Reconcile: session.completed_tasks <-> todo({ action: "list" }) status (bidirectional sync)
...
7. Create missing tasks, set dependencies via todo({ action: "update", addBlockedBy })
