...
3. Use CLI tools (e.g., `teammate({ agent: "delegate", taskType: "analysis", task: "...", /* --to agy: set model via model-availability */ }) or direct tools (Glob, Grep) to scan for existing interactive components, animation patterns, event handling approaches
