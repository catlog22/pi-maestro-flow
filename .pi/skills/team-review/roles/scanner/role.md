...
Execute via `teammate({ agent: "general", taskType: "analysis", /* --rule analysis-review-code-quality */ })`. Parse JSON array response, validate required fields (dimension, title, location.file), enforce per-dimension limit (max 5 each), filter minimum severity (medium+). Write `{run_dir}/outputs/scan/semantic-findings.json`.
