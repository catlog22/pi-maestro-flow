...
4.5. **evaluateSpecialistInjection** (based on detected codebase characteristics):
   - If callback from analyst, planner, or executor role:
     ...
     d. P0 matches → todo({ action: "create" }) with blockedBy on current stage, blocks downstream
     e. P1 matches → todo({ action: "create" }) parallel with REVIEW/TEST stage
...
3. For each IMPL task: todo({ action: "create" }) with structured description (dispatch.md template)
4. Set blockedBy:
...
3. SendMessage to supervisor (see SKILL.md Supervisor Wake Template):
   ...
   ```
4. Do NOT todo({ action: "update" }) in_progress — supervisor claims the task itself
