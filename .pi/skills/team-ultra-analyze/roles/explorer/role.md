...
**Primary: `teammate({ agent: "explorer" })`** (structured multi-prompt):
...
**Fallback: `teammate`** (when explore results insufficient):

```bash
teammate({ agent: "delegate", taskType: "analysis", task: "PURPOSE: Explore codebase for <topic> from <perspective> perspective\nTASK: Search for topic-related patterns | Identify…", prompt: "analysis-analyze-code-patterns" })
```
