...
> 裸名称为 first-tier step：经 `/maestro "<意图>"` 自动路由，或 `maestro run start "<intent>" --cmd <step> --platform pi ...` 直接执行（v2 legacy；session/3.0 workspace 见 run-mode.md v3 章节）；`/` 前缀为独立命令。
