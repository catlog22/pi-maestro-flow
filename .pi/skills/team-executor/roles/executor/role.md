...
- Reconcile session state with todo({ action: "list" }) on startup
...
### Step 3: Reconcile with todo({ action: "list" })

Compare todo({ action: "list" }) with session.completed_tasks, bidirectional sync.
...
For each task in task-analysis, check if exists in todo({ action: "list" }), create if missing.
