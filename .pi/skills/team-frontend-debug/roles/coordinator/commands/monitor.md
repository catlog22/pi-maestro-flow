...
2. Create REPRODUCE-002 task:
   - todo({ action: "create" }) with description from Analyzer's request
   - todo({ action: "update" }) to set owner (no blockedBy — can start immediately)
3. Create ANALYZE-002 task:
   - todo({ action: "create" }) + todo({ action: "update" }) with addBlockedBy: [REPRODUCE-002]
   - todo({ action: "update" }) FIX-001 with addBlockedBy to include ANALYZE-002
