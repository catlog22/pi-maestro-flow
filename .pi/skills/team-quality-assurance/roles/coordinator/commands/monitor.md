...
4. Completion -> mark task done via todo({ action: "update" })(status="completed"), remove from active_workers
...
2. Has active -> check each status
   - completed -> mark done via todo({ action: "update" })
...
1. Collect from todo({ action: "list" }):
...
| QAANA-* | analyst | false |
   ...
   c. todo({ action: "update" }) -> in_progress
