...
function convertCommandFrontmatter(commandFm, config) {
  return {
    ...
    // "Skill(*), TodoWrite(*), Read(*)" → "Task, user prompt, TodoWrite, Read, Write, Edit, Bash, Glob, Grep, Skill"
  };
}
...
function expandToolWildcards(toolsStr) {
  ...
  const baseTools = ['Task', 'user prompt', 'TodoWrite', 'Read', 'Write', 'Edit', 'Bash', 'Glob', 'Grep'];
