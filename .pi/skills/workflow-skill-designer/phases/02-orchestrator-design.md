...
Generate the SKILL.md orchestrator file from workflowConfig, applying all coordination patterns (progressive loading, todo({ action: "update" }), data flow, conditional execution).
...
- Build todo({ action: "update" }) attachment/collapse patterns from phase definitions
...
function expandToolWildcards(toolsStr) {
  const expanded = toolsStr
    ...
    .replace(/todo({ action: "update" })\(\*\)/g, 'todo({ action: "update" })')
    .replace(/Read\(\*\)/g, 'Read')
    .replace(/Bash\(\*\)/g, 'Bash')
    .replace(/Glob\(\*\)/g, 'Glob')
    .replace(/Grep\(\*\)/g, 'Grep')
...
## Step 2.6: Generate todo({ action: "update" }) Pattern

```javascript
function generateTodoWritePattern(config) {
  let section = '## todo({ action: "update" }) Pattern\n\n';
  ...
  section += '   - Sub-tasks are **attached** to orchestrator\'s todo({ action: "update" })\n';
  ...
  // Generate todo({ action: "update" }) examples for phases with sub-tasks
