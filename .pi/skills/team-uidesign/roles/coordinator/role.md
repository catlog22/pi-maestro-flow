...
1. Resolve workspace paths (MUST do first):
   ...
   - `skill_root` = `<project_root>/.pi/skills/team-uidesign`
...
1. **Resolve Run** (birth-packet first): if the dispatch context already carries `run_id` / `run_dir` (injected by an orchestrator), store them in `team-session.json` and skip create — a second create mints an empty duplicate Run. Otherwise: `maestro run start "<task summary>" --cmd team-uidesign --session <slug> --platform pi --workflow-root .`
